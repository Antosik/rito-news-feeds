...
func valStatusEntryToFeedEntry(entry wr.StatusEntry) internal.FeedEntry {
...
func createWrStatusFeed(regionID string, locale statusLocale, entries []wr.StatusEntry) internal.Feed {
	...
	links := internal.FeedLinks{
		Alternate: fmt.Sprintf("https://status.riotgames.com/wr?region=%s&locale=%s", regionID, locale.Locale),
	}
