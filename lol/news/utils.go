...
func lolNewsEntryToFeedEntry(entry lol.NewsEntry) internal.FeedEntry {
...
func createLolNewsFeed(parameters newsParameters, entries []lol.NewsEntry) internal.Feed {
	feedEntries := make([]internal.FeedEntry, len(entries))
	for i, entry := range entries {
		feedEntries[i] = lolNewsEntryToFeedEntry(entry)
	}
	...
	links := internal.FeedLinks{
		Alternate: fmt.Sprintf("https://www.leagueoflegends.com/%s/news/", strings.ToLower(parameters.Locale)),
	}
...
func compareLolNewsEntry(a lol.NewsEntry, b lol.NewsEntry) bool {
	return a.UID == b.UID
}
